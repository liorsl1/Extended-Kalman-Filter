...
void filter(VectorXd &x, MatrixXd &P) {
	...
	for (unsigned int n = 0; n < measurements.size(); ++n) {
        ...
        VectorXd y = z - H*x; // gives us a number of difference (becuase H = (1,0)) between sensor measure (z) and state (x)
		// new state - new updated states (of x(estimated state) and P(uncertainty)) according to the measurements above
		x = x + (K*y);
        P = (I - K*H)*P;
		// KF Prediction step
        x = F*x + u;
